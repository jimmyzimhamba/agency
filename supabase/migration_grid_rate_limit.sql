...
-- bypass by simply not sending a header. This table is that "somewhere" —
-- it's only ever read/written by the review-* Edge Functions using the
-- service-role key (see supabase/functions/review-load, review-update,
-- review-reorder, review-complete). Deliberately NO RLS policies at all —
-- not even for `authenticated` — so nobody, including a signed-in teammate,
