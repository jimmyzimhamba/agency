...
--     business name, area, and the specific gap you noted about them, --     so instead of a generic template, every agent's first message reads
--     like it was written for that exact business.
--   - A human on your team still reviews and taps Send inside WhatsApp, --     nothing sends itself. This just removes the blank-page problem so
...
insert into public.message_templates (title, category, body, niche_id, sort_order)
select 'General Opener', 'opener', E'Hi! I came across {{business_name}}{{area_clause}}, {{gap_clause}}. We help local Harare businesses grow through social media and WhatsApp marketing. Would you be open to a quick chat?\n\n{{agent_name}}, Studio X Marketing', null, 20
