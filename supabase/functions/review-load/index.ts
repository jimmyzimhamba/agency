...
//   load a grid plan by its share_token. Nobody calling this is signed in, //   there's no user session, no profile, nothing, so this function is the
...
//
//   Media: grid-media is a PRIVATE bucket (see migration_grid_plans.sql), //   there is no public URL for any file in it. This function mints a
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // First time the client actually opens a shared-but-unopened plan,     // record it, both as a plan-level status flip and as an audit entry.
