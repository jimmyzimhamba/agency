...
//   The "Phoenix AI" tab lets a teammate type a question like "who hasn't been
...
function buildSystemPrompt(callerName: string, callerRole: string) {
  ...
  return `You are Phoenix AI, an assistant built into Studio X Command, the internal sales tool for Studio X Marketing, a digital marketing agency in Harare, Zimbabwe. You're talking to ${callerName} (role: ${callerRole}). Today's date is ${today}.
...
Deno.serve(async (req: Request) => {
  ...
  if (!ANTHROPIC_API_KEY) {
    return json({ error: "Phoenix AI isn't set up yet. Ask the owner to add an Anthropic API key in Supabase (same one used for AI Research)." }, 501);
  }
  ...
  try {
    ...
    if ((count || 0) >= RATE_LIMIT_PER_HOUR) {
      return json(
        { error: `Phoenix AI limit reached (${RATE_LIMIT_PER_HOUR} messages/hour). This keeps API costs in check. Try again shortly.` },
        429
      );
    }
